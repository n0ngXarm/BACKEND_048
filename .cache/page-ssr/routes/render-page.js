...
// EXTERNAL MODULE: external "C:\\Users\\armmy\\Desktop\\BACKEND_048\\node_modules\\react\\index.js"
var external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_ = __webpack_require__(8238);
var external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_default = /*#__PURE__*/__webpack_require__.n(external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_);
...
const getSliceId=(sliceName,sliceProps)=>{if(!Object.keys(sliceProps).length){return sliceName;}const propsString=createContentDigest(sliceProps);return`${sliceName}-${propsString}`;};const ServerSlice=({sliceName,allowEmpty,children,...sliceProps})=>{const slicesMap=(0,external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_.useContext)(context.SlicesMapContext);const slicesProps=(0,external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_.useContext)(context.SlicesPropsContext);const concreteSliceName=slicesMap[sliceName];if(!concreteSliceName){if(allowEmpty){return null;}else{throw new Error(`Slice "${concreteSliceName}" for "${sliceName}" slot not found`);}}const sliceId=getSliceId(concreteSliceName,sliceProps);// set props on context object for static-entry to return
let sliceUsage=slicesProps[sliceId];if(!sliceUsage){slicesProps[sliceId]=sliceUsage={props:sliceProps,sliceName:concreteSliceName,hasChildren:!!children};}else{if(children){sliceUsage.hasChildren=true;}}return/*#__PURE__*/external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_default().createElement(server_slice_renderer.ServerSliceRenderer,{sliceId:sliceId},children);};
;// ./.cache/slice/inline-slice.js
const InlineSlice=({sliceName,allowEmpty,children,...sliceProps})=>{const slicesMap=(0,external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_.useContext)(context.SlicesMapContext);const slicesResultsMap=(0,external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_.useContext)(context.SlicesResultsContext);const concreteSliceName=slicesMap[sliceName];const slice=slicesResultsMap.get(concreteSliceName);if(!slice){if(allowEmpty){return null;}else{throw new Error(`Slice "${concreteSliceName}" for "${sliceName}" slot not found`);}}return/*#__PURE__*/external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_default().createElement(slice.component,Object.assign({sliceContext:slice.sliceContext,data:slice.data},sliceProps),children);};
;// ./.cache/slice.js
"use client";function Slice(props){if(true){// we use sliceName internally, so remap alias to sliceName
const internalProps={...props,sliceName:props.alias};delete internalProps.alias;delete internalProps.__renderedByLocation;const slicesContext=(0,external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_.useContext)(context.SlicesContext);// validate props
const propErrors=validateSliceProps(props);if(Object.keys(propErrors).length){throw new SlicePropsError(slicesContext.renderEnvironment===`browser`,internalProps.sliceName,propErrors,props.__renderedByLocation);}if(slicesContext.renderEnvironment===`server`){return/*#__PURE__*/external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_default().createElement(ServerSlice,internalProps);}else if(slicesContext.renderEnvironment===`browser`){// in the browser, we'll just render the component as is
return/*#__PURE__*/external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_default().createElement(InlineSlice,internalProps);}else if(slicesContext.renderEnvironment===`engines`||slicesContext.renderEnvironment===`dev-ssr`){// if we're in SSR, we'll just render the component as is
return/*#__PURE__*/external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_default().createElement(InlineSlice,internalProps);}else if(slicesContext.renderEnvironment===`slices`){// we are not yet supporting nested slices
...
const fullStack=external_C_Users_armmy_Desktop_BACKEND_048_node_modules_react_index_js_default().__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.ReactDebugCurrentFrame.getCurrentStack();// remove the first line of the stack trace
...
var _react = _interopRequireDefault(__webpack_require__(8238));
...
const React=__webpack_require__(8238);const{grabMatchParams}=__webpack_require__(5870);const{StaticQueryContext}=__webpack_require__(123);const{headExportValidator,filterHeadProps,isElementType,isValidNodeName,warnForInvalidTag}=__webpack_require__(9226);const{ServerLocation,Router}=__webpack_require__(6462);const{renderToString}=__webpack_require__(9250);const{parse}=__webpack_require__(6192);const styleToOjbect=__webpack_require__(9108);function applyHtmlAndBodyAttributesSSR(htmlAndBodyAttributes,{setHtmlAttributes,setBodyAttributes}){if(!htmlAndBodyAttributes)return;const{html,body}=htmlAndBodyAttributes;setHtmlAttributes(html);setBodyAttributes(body);}function getValidHeadNodesAndAttributesSSR(rootNode,htmlAndBodyAttributes={html:{},body:{}}){const seenIds=new Map();const validHeadNodes=[];// Filter out non-element nodes before looping since we don't care about them
...
/* harmony export */ });
/* harmony import */ var react__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(8238);
...
var React = _interopRequireWildcard(__webpack_require__(8238));
...
var _react = __webpack_require__(8238);
 ...
 * LICENSE file in the root directory of this source tree.
 */
var aa=__webpack_require__(2460),e=__webpack_require__(8238),k=null,l=0,m=!0;function p(a,b){a=a.write(b);m=m&&a}
...
"use strict";
module.exports = require("C:\\Users\\armmy\\Desktop\\BACKEND_048\\.cache\\ssr-builtin-trackers\\fs");
...
"use strict";
module.exports = require("C:\\Users\\armmy\\Desktop\\BACKEND_048\\node_modules\\react\\index.js");
...
const React=__webpack_require__(8238);const path=__webpack_require__(2003);const{renderToStaticMarkup,renderToPipeableStream}=__webpack_require__(9250);const{ServerLocation,Router,isRedirect}=__webpack_require__(6462);const merge=__webpack_require__(4744);const{StaticQueryContext}=__webpack_require__(123);const fs=__webpack_require__(7661);const{WritableAsPromise}=__webpack_require__(5297);const{RouteAnnouncerProps}=__webpack_require__(6858);const{apiRunner,apiRunnerAsync}=__webpack_require__(9432);const asyncRequires=__webpack_require__(6877);const{version:gatsbyVersion}=__webpack_require__(5818);const{grabMatchParams}=__webpack_require__(5870);const{headHandlerForSSR}=__webpack_require__(988);const{SlicesResultsContext,SlicesContext,SlicesMapContext,SlicesPropsContext}=__webpack_require__(2024);const{ServerSliceRenderer}=__webpack_require__(8455);// we want to force posix-style joins, so Windows doesn't produce backslashes for urls
